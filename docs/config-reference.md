...
| `max_bot_turns` | u32 | `100` | Max consecutive bot turns per thread before throttling (soft limit). Human message resets the counter. The default hard floor is 1000 consecutive bot messages; configuring `max_bot_turns` above 1000 raises the effective hard cap to that configured value. |
