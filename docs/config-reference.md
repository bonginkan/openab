...
| `trusted_bot_role_ids` | string[] | `[]` | Discord role IDs that trust bot authors holding any listed role. Combines with `trusted_bot_ids` using OR. This checks the sender's roles; it does not make a role mention a trigger. |
