...
| `trusted_bot_role_ids` | string[] | `[]` | Discord role IDs that trust bot authors holding any listed role. Combines with `trusted_bot_ids` using OR and grants the same admission override: a trusted bot that directly @mentions this bot bypasses `allow_bot_messages`, including `"off"`. This checks the sender's roles; it does not make a role mention a trigger. |
