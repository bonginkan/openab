...
pub fn format_user_error(message: &str) -> String {
    ...
    if msg_lower.contains("session load timeout") {
        return "**Session Load Timeout**\nThe previous session could not be restored, so your message was not sent. Use `/reset` and try again.".to_string();
    }
    if msg_lower.contains("session load connection lost") {
        return "**Session Load Failed**\nThe previous session could not be restored because the agent connection was lost, so your message was not sent. Use `/reset` and try again.".to_string();
    }
...
mod tests {
    ...
    fn format_user_error_session_load_connection_lost() {
        let result =
            format_user_error("session load connection lost: could not restore previous session");
